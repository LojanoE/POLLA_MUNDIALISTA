...
| `admin.html` + `js/admin.js` | Admin enters real results, **generates knockout bracket automatically from group results**, enables final phase, recalculates scores, manages users |
...
Each `partidos_final` doc includes `source_equipo1` and `source_equipo2` fields pointing to the previous match ID (e.g., F17's source_equipo1 = "F1").
...
- **🗃️ Inicializar Base de Datos:** Load all 72 group + 32 knockout matches into Firestore.
- **⚠️ Reiniciar Todo:** Delete all matches, predictions, and config (keep users).
...
All CSS and JS references use `?v=X.0` query parameters to force browser refresh during testing. Bump this version (e.g., `?v=3.0` → `?v=4.0`) when making major structural changes to ensure users get the latest files without hard-refreshing.
